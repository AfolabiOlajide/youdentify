...
export default function CreateProfile() {
    ...
    const { getUserByAddress, getUsernameByAddress, createUser, isQueryError, isQuerySuccess } = useQueries();
    ...
    useEffect(() => {
        const getUserInfo = async () => {
            let userInfo = await getUserByAddress(account?.address as string);
            let username = await getUsernameByAddress(
                account?.address as string
            );
            if (username !== null || username !== undefined || username !== "") {
                setFormData({
                    first_name: (userInfo as ReturnDataType)?.basicInfo.firstName,
                    last_name: (userInfo as ReturnDataType)?.basicInfo.lastName,
                    username: username || "",
                    email: (userInfo as ReturnDataType)?.basicInfo.email,
                    home_address: (userInfo as ReturnDataType)?.basicInfo.homeAddress,
                    date_of_birth: (userInfo as ReturnDataType)?.basicInfo.dateOfBirth,
                    education: (userInfo as ReturnDataType)?.professionalInfo.education,
                    work_history: (userInfo as ReturnDataType)?.professionalInfo.workHistory,
                    phone_number: (userInfo as ReturnDataType)?.basicInfo.phoneNumber,
                    job_title: (userInfo as ReturnDataType)?.professionalInfo.jobTitle,
                    x: (userInfo as ReturnDataType)?.socialLinks.x,
                    instagram: (userInfo as ReturnDataType)?.socialLinks.instagram,
                    tiktok: (userInfo as ReturnDataType)?.socialLinks.tiktok,
                    youtube: (userInfo as ReturnDataType)?.socialLinks.youtube,
                    linkedin: (userInfo as ReturnDataType)?.socialLinks.linkedIn,
                    info: (userInfo as ReturnDataType)?.professionalInfo.info,
                    skills: (userInfo as ReturnDataType)?.professionalInfo.skills,
                    imageUrl: (userInfo as ReturnDataType)?.professionalInfo.imageUrl,
                });
    ...
    const handleImagesChange = async (files: File[]) => {
        ...
        try {
            ...
            const options = {
                ...
                headers: {
                    Authorization:
                        `Bearer ${process.env.NEXT_PUBLIC_PINATA_JWT_SECRET}`,
                },
                body: form,
            };
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const professionalInfo: ProfessionalInfoType = {
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between space-y-4 md:space-y-0 md:space-x-10 pt-20 pb-20 px-4 md:px-16">
            <div className="md:sticky md:top-[10%]">
                <div className="md:text-4xl text-xl font-medium w-3/3 pb-3">
                    ...
                    <span className="text-sky-500">YouDentifi</span>
                </div>
                <Toaster />
                <UserProfileDisplay
                    formData={formData}
                        ...
                        <div className="text-xl md:text-2xl flex items-center justify-center flex-colpx-8">
                            <div className="w-80">
                                ...
                                <div className="text-gray-500 font-light text-center justify-center mx-auto py-10">
                                    We&apos;ve received your inquiry and will be
